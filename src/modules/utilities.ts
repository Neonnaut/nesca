...
function resolve_nested_categories(
  ...
  function tokenize(expr: string): (string | { group: string; weight: number })[] {
    ...
    while (i < expr.length) {
      if (expr[i] === '[') {
        ...
        // Capture nested bracket structure
        let depth = 1, j = i + 1;
        ...
        // Handle optional weight annotation like [x y]*2
        let weight = 1;
        if (expr[i] === '*') {
          i++;
          let w = '';
  ...
  function evaluate(expr: string, multiplier = 1): Entry[] {
    ...
    const dist = get_distribution(tokens.length, default_distribution);
    const entries: Entry[] = [];
    ...
    for (let i = 0; i < tokens.length; i++) {
      ...
      if (typeof token === 'string') {
        // Handle inline weight: "key*2"
        const [key, rawWeight] = token.split('*');
        const weight = rawWeight === undefined || rawWeight === '' ? 1 : parseFloat(rawWeight);
        entries.push({ key: key.trim(), weight: weight * tokenWeight });
      } else {
        // Recursively evaluate group content
        ...
        for (const { key, weight } of innerEntries) {
          entries.push({
            key,
            weight: (weight / total) * token.weight * tokenWeight
          });
        }
      }
    }

    // Collapse duplicate keys: sum weights by key
    const map = new Map<string, number>();
    for (const { key, weight } of entries) {
      map.set(key, (map.get(key) || 0) + weight);
    }

    return Array.from(map, ([key, weight]) => ({ key, weight }));
  }

  // Final output: separate key and weight arrays
